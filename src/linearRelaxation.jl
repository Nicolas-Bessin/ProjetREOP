...
function linearRelaxation(instance :: Instance,  filename :: String = "")
    ...
    @variable(model, x[1:nbSubLocations, 1:nbSubTypes] >= 0)

    for v in 1:nbSubLocations
        for s in 1:nbSubTypes
            @constraint(model, x[v, s] <= 1)
        end
    end
    ...
    @variable(model, yland[1:nbSubLocations, 1:nbLandCableTypes] >= 0)

    for v in 1:nbSubLocations
        for q in 1:nbLandCableTypes
            @constraint(model, yland[v, q] <= 1)
        end
    end
    ...
    @variable(model, z[1:nbSubLocations, 1:nbTurbines] >= 0)

    for v in 1:nbSubLocations
        for t in 1:nbTurbines
            @constraint(model, z[v, t] <= 1)
        end
    end
    ...
    @variable(model, ysub[1:nbSubLocations, 1:nbSubLocations, 1:nbSubCableTypes] >= 0)

    for v1 in 1:nbSubLocations
        for v2 in 1:nbSubLocations
            for i in 1:nbSubCableTypes
                @constraint(model, ysub[v1, v2, i] <= 1)
            end
        end
    end
    ...
    return solution, time, value.(constructionCost), value.(operationalCost), value.(constructionCost) + value.(operationalCost)
end