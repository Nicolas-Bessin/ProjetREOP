...
\end{table}
Le nombre de variable / contrainte - et donc le temps de résolution - du problème est donc en $O(|V^s| |S| + |V^s|^2 |Q^s| + |V^s| |Q^0| + |V^s| |V^t| + |V^s| ^ 2 |\Omega|)$. \\
...
Ces sous-instance étant extraites des instances initiales, la solution optimale de la sous-instance est une solution réalisable pour l'instance initiale, de coût supérieur à l'optimal.
...
\textit{Le code source est disponible sur \underline{\href{https://github.com/Nicolas-Bessin/ProjetREOP}{github}}.}
